...
r0z 是一个独立实现的 zygisy，提供对 zygisy API 的支持，可用于 KernelSU，并可替代 Magisk 内置的 zygisy。
...
+ 提供 WebUI 和 `r0z-ctl`，用于状态查看和 hide list 管理
...
+ 在 Magisk 环境下，安装 r0z 前必须先关闭内置 r0z。
...
r0z 只保证 zygisy API 层面的行为兼容，不保证 Magisk 内部私有功能也保持一致。
...
+ 通过 WebUI 或 `bin/r0z-ctl hide-list ...` 管理 hide list
...
+ Dr-TSNG / ZygiskNext：项目目录结构以及第一版实现参考了它的代码实现
